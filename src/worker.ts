...
const worker = {
	fetch(request: Request, env: WorkerEnvironment): Promise<Response> {
		if (env.RATE_LIMIT_KV && (!cachedRateLimitStorage || cachedRateLimitKvNamespace !== env.RATE_LIMIT_KV)) {
		...
		return proxyRequest(request, {
			...env,
			RATE_LIMIT_STORAGE: env.RATE_LIMIT_KV ? cachedRateLimitStorage : undefined,
		});
	},
};
